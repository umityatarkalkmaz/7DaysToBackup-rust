...
pub struct Strings {
    ...
    pub restore_progress: &'static str,
    pub restore_confirm: &'static str,
    pub restore_success: &'static str,
    pub restore_error: &'static str,
    pub delete_backup_confirm: &'static str,
    pub backup_selection_error: &'static str,
    pub auto_backup_label: &'static str,
    pub auto_backup_help: &'static str,
    pub auto_backup_keep_label: &'static str,
    pub auto_backup_done: &'static str,
...
pub static TR: Strings = Strings {
    ...
    backup_list: "Yedek Geçmişi",
    no_backups: "Bu save'in yedeği yok",
    restore: "Geri Yükle",
    delete_backup: "Yedeği Sil",
    restore_progress: "Yedek geri yükleniyor...",
    restore_confirm: "'{}' yedeği geri yüklenecek.\n\nSave'in şu anki hâli silinmiyor: yeni bir yedeğe dönüştürülüyor, dolayısıyla bu işlemden geri dönebilirsiniz.\n\nDevam edilsin mi?",
    restore_success: "Yedek geri yüklendi. Save'in önceki hâli geçmişte yeni bir kayıt olarak duruyor.",
    restore_error: "Geri yükleme başarısız:",
    delete_backup_confirm: "'{}' yedeği kalıcı olarak silinecek. Devam edilsin mi?",
    backup_selection_error: "Lütfen geçmişten bir yedek seçin!",
    auto_backup_label: "Otomatik yedekleme (dakika)",
    auto_backup_help: "0 = kapalı. Seçili save, uygulama açıkken bu aralıkla yedeklenir; elle başlattığınız bir işlem sürerken tur atlanır ve pencere açılmaz.",
    auto_backup_keep_label: "Saklanacak yedek",
    auto_backup_done: "Otomatik yedek alındı ({})",
...
pub static EN: Strings = Strings {
    ...
    no_backups: "This save has no backups",
    restore: "Restore",
    delete_backup: "Delete Backup",
    restore_progress: "Restoring backup...",
    restore_confirm: "The backup '{}' will be restored.\n\nThe save's current state is not deleted: it becomes a new backup, so you can undo this.\n\nContinue?",
    restore_success: "Backup restored. The save's previous state is kept as a new entry in the history.",
    restore_error: "Restore failed:",
    delete_backup_confirm: "The backup '{}' will be permanently deleted. Continue?",
    backup_selection_error: "Please select a backup from the history!",
    auto_backup_label: "Automatic backup (minutes)",
    auto_backup_help: "0 = off. The selected save is backed up at this interval while the app is open; a round is skipped while an operation you started is running, and no window opens.",
    auto_backup_keep_label: "Backups to keep",
    auto_backup_done: "Automatic backup taken ({})",
...
impl Strings {
    ...
    pub(crate) fn fields(&self) -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            ("no_backups", self.no_backups),
            ("restore", self.restore),
            ("delete_backup", self.delete_backup),
            ("restore_progress", self.restore_progress),
            ("restore_confirm", self.restore_confirm),
            ("restore_success", self.restore_success),
            ("restore_error", self.restore_error),
            ("delete_backup_confirm", self.delete_backup_confirm),
            ("backup_selection_error", self.backup_selection_error),
            ("auto_backup_label", self.auto_backup_label),
            ("auto_backup_help", self.auto_backup_help),
            ("auto_backup_keep_label", self.auto_backup_keep_label),
            ("auto_backup_done", self.auto_backup_done),
